...
// HogQL: total pageviews + distinct sessions across all time.
const STATS_QUERY =
  "SELECT count() AS pageviews, count(DISTINCT $session_id) AS sessions FROM events WHERE event = '$pageview'"
...
async function fetchPostHogStats(): Promise<{ pageviews: number; sessions: number }> {
  ...
  return { pageviews: row?.[0] ?? 0, sessions: row?.[1] ?? 0 }
}
